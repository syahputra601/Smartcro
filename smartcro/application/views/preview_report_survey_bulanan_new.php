            ...
            if($KodeKantor == @$GetKodeKantor){
                ...
                foreach($DataCabang as $row_cabang){
                    ...
                    if($get_month_filter != '' && $get_year_filter != ''){
                        $DataReportBulanan = $this->db->query("SELECT COUNT(c.kode_kantor) AS jml_data_area, COUNT(e.type) AS jml_data, a.nama_cust, c.kode_kantor, a.id_comp, c.nama AS nama_cabang, c.kota, e.no_type, e.type, e.quest, e.nm_quest, COUNT(IF(d.nilai = '0', 0, NULL)) 'Kosong', COUNT(IF(d.nilai = '1', 0, NULL)) 'Pertama', COUNT(IF(d.nilai = '2', 0, NULL)) 'Dua', COUNT(IF(d.nilai = '3', 0, NULL)) 'Tiga', COUNT(IF(d.nilai = '4', 0, NULL)) 'Empat', COUNT(IF(d.nilai = '5', 0, NULL)) 'Lima', COUNT(IF(d.nilai = '6', 0, NULL)) 'Enam', COUNT(IF(d.nilai = '7', 0, NULL)) 'Tujuh', COUNT(IF(d.nilai = '8', 0, NULL)) 'Delapan', COUNT(IF(d.nilai = '9', 0, NULL)) 'Sembilan', COUNT(IF(d.nilai = '10', 0, NULL)) 'Sepuluh', (SUM(0)+SUM(1)) AS Total, (SUM('0')+SUM('3')) AS Total_A FROM tb_surv AS a INNER JOIN tb_hedsurv AS b ON a.id_surv = b.id_surv INNER JOIN mst_company AS c ON a.id_comp = c.id_company INNER JOIN tb_detsurv AS d ON a.id_surv = d.id_surv INNER JOIN mst_quest AS e ON d.id_quest = e.id_quest WHERE c.kode_kantor = '".$KodeKantor."' AND c.nama = '".$CabangNama."' AND b.id_respon = '6' AND a.tgl_telp LIKE '%".$join_month_year_filter."%' GROUP BY d.id_quest ORDER BY a.tipe_cust ASC, e.no_type ASC")->result();
                    }else{
